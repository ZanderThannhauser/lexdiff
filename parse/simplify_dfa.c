...
static struct regexset* build_universe(struct regex* start)
{
	ENTER;
	...
	while (quack_is_nonempty(todo))
	{
		...
		{
			struct regex* to = node->transitions[i];
...
static struct regex* find(struct avl_tree_t* connections, struct regex* a)
{
	...
	return regexset_get_head(sa->set);
}
...
struct regex* simplify_dfa(struct regex* original)
{
	ENTER;
	
	struct avl_tree_t* dependent_of = avl_alloc_tree(compare_dependent_of_nodes, free_dependent_of_node);
	
	struct regexset* universe = build_universe(original);
